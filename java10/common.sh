...
function checkIfJDKIsDownloaded() {
    ...
    if [[ $JDK_NAME_GENERIC == "jdk-10+" ]]; then
  		echo "No JDK does not exist, downloading now..."
      wget --no-check-certificate --header "Cookie: oraclelicense=accept-securebackup-cookie" -O $JDK_TAR_FILE_NAME "${JDK_DOWNLOAD_BASE_URL}/java/jdk10/archive/${BUILD_NUMBER}/binaries/${JDK_TAR_FILE_NAME}"
    fi
  else
...
function checkIfTheRightJava10CommandsAreBeingExecuted() {
	echo ""
	echo "Running javac with the -version option, to verify if the right JDK 10 commands are being executed."
	javac -version
	
	echo ""
	echo "Running java with the -version option, to verify if the right JDK 10 commands are being executed."
	java -version	
}
