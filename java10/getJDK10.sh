...
JDK_DOWNLOAD_HOME_PAGE="http://jdk.java.net/10/"
BUILD_NUMBER=$(curl $JDK_DOWNLOAD_HOME_PAGE/ | grep "Most recent build" | awk '{print $4}' |  tr -d "</h2>" | perl -nle 'print$& if m{jdk\-10\+\K\w+}')
...
function checkIfJDKIsInstalled() {
	...
	if [[ "$OSTYPE" == "darwin"* ]]; then
		...
		echo "Please mount ${JDK_TAR_FILE_NAME} on Finder and install JDK 10. It will be installed at ${JDK_DESTINATION}/${JDK_FOLDER_NAME}."
		return
	fi
...
checkIf_PATH_IsSet
checkIfTheRightJava10CommandsAreBeingExecuted
